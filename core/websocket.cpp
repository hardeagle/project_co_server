...
CWebsocket::CWebsocket(const string& url, bool use_mask) {
    if (url.size() >= WS_ADDR_BUF_SIZE) {
        LOG << "[ERR]url size limit exceeded: " << url.c_str();
        return;
    }
    m_url = url;
    ...
    if (sscanf(url.c_str(), "ws://%[^:/]:%d/%s", m_host, &m_port, m_path) == 3) {
        ...
        LOG << "[ERR]Could not parse WebSocket url: " << url.c_str();
        return;
    }
    LOG << "[INF]Parsed url: host=, port, path" << m_host << m_port << m_path;
    m_use_mask  = use_mask;
    m_ws_state  = OPEN;
    m_is_rx_bad = false;
}
...
int CWebsocket::connect_hostname() {
    ...
    if (ret != 0) {
        LOG << "[ERR]Failed to getaddrinfo: " << gai_strerror(ret);
        return LIGHTWS_ERR_CONNECT;
    }

    for (p = result; p != NULL; p = p->ai_next) {
        m_sock_fd = socket(p->ai_family, p->ai_socktype, 0);
        if (m_sock_fd == INVALID_SOCKET) {
            continue;
        }
        ret = connect(m_sock_fd, p->ai_addr, p->ai_addrlen);
        if (ret != SOCKET_ERROR) {
            LOG << "connect fail ret " << ret;
            break;
        }
    ...
    if (m_sock_fd == INVALID_SOCKET) {
        LOG << "[ERR]Unable to connect to " << m_host << m_port;
        return LIGHTWS_ERR_CONNECT;
    }
    ...
    // Websocket handshake
    {
        ...
        ::send(m_sock_fd, req.c_str(), req.size(), 0);

        for (i = 0; i < 2 || (i < 1023 && line[i-2] != '\r' && line[i-1] != '\n'); ++i) { 
            if (recv(m_sock_fd, line+i, 1, 0) == 0) { 
                LOG << "12321312321312fial";
                return LIGTHWS_ERR_HANDSHAKE; 
            } 
        }
        line[i] = 0;
        if (i == 1023) { 
            LOG << "ERROR: Got invalid status line connecting to " << m_url.c_str(); 
            return LIGTHWS_ERR_HANDSHAKE; 
        }
        if (sscanf(line, "HTTP/1.1 %d", &status) != 1 || status != 101) { 
            LOG << "ERROR: Got bad status connecting to: " << m_url.c_str() << line; 
            return LIGTHWS_ERR_HANDSHAKE; 
        }
